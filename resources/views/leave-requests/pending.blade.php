...
<style>
    ...
    .filter-bar {
        ...
        margin-bottom: 16px; background: var(--white); padding: 12px 16px;
    ...
    .filter-select:focus, .filter-input:focus { border-color: var(--primary); background: white; box-shadow: 0 0 0 3px var(--primary-light); }
    .filter-btn {
...
<div class="page-header animate-in">
    <div>
        ...
        <p class="page-subtitle">Validez les congés de vos collaborateurs</p>
    </div>
</div>
...
<div class="kpi-grid">
    ...
    <div class="kpi-card animate-in delay-2">
        ...
        <div class="kpi-value">{{ $recentCount ?? 0 }}</div>
        <div class="kpi-footer"><i class="fas fa-arrow-up" style="color:#10B981;"></i> Derniers 7 jours</div>
    </div>
    <div class="kpi-card animate-in delay-3">
        ...
        <div class="kpi-footer"><i class="fas fa-user-friends" style="color:var(--primary);"></i> Concernés</div>
    </div>
</div>

{{-- Filtre --}}
<div class="filter-bar animate-in delay-1">
    <form method="GET" action="{{ route('leave-requests.pending') }}">
        ...
        <input type="date" name="start_date" class="filter-input" value="{{ request('start_date') }}" placeholder="Début">
        <input type="date" name="end_date" class="filter-input" value="{{ request('end_date') }}" placeholder="Fin">
        <button type="submit" class="filter-btn"><i class="fas fa-filter"></i> Filtrer</button>
        @if(request()->anyFilled(['employee_id','leave_type_id','start_date','end_date']))
            <a href="{{ route('leave-requests.pending') }}" class="reset-btn"><i class="fas fa-times"></i> Réinitialiser</a>
        @endif
    </form>
</div>
    ...
    <div class="table-card animate-in delay-2">
                        ...
                        <tr>
                            <td>
                                <div style="display:flex; align-items:center; gap:8px;">
                                    <div style="width:28px; height:28px; border-radius:8px; background:var(--primary); color:white; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:12px;">
                                        {{ strtoupper(substr($req->employee->user->name, 0, 1)) }}
                                    </div>
                                    <strong>{{ $req->employee->user->name }}</strong>
                                </div>
                            </td>
                            <td><span class="badge">{{ $req->leaveType->name }}</span></td>
                            <td>{{ $req->start_date->format('d/m/Y') }} - {{ $req->end_date->format('d/m/Y') }}</td>
                            <td>{{ \Illuminate\Support\Str::limit($req->reason, 40) ?: '—' }}</td>
                            <td style="text-align:right;">
                                <a href="{{ route('leave-requests.show', $req) }}" class="action-btn" title="Traiter">
        ...
        @else
            <div class="empty-state">
                ...
                <p style="font-weight:500;">Aucune demande en attente.</p>
            </div>
        @endif
    </div>
    ...
    <div class="guide-card animate-in delay-2">
        ...
        <div class="guide-item">
            ...
            <div class="guide-text">
                ...
                <p>Voyez tous les détails d'une demande.</p>
            </div>
        </div>
        <div class="guide-item">
            <div class="guide-icon"><i class="fas fa-check-circle"></i></div>
            <div class="guide-text">
                <strong>Décidez</strong>
                <p>Approuvez ou refusez la demande.</p>
            </div>
        </div>
        <div class="guide-item">
            <div class="guide-icon"><i class="fas fa-bell"></i></div>
            <div class="guide-text">
                <strong>Notifié</strong>
                <p>L'employé reçoit la décision.</p>
            </div>
        </div>
    </div>
</div>
