...
<div class="page-header animate-in">
    <h1 class="page-title"><i class="fas fa-upload" style="color:var(--primary); margin-right:6px;"></i> <span>Téléverser un document</span></h1>
...
<div class="content-layout">
    ...
    <div class="form-card animate-in delay-1">
        <form method="POST" action="{{ route('admin.documents.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="form-label"><i class="fas fa-tag"></i> Type <span style="color:var(--primary);">*</span></label>
                <select name="type" required class="form-select @error('type') is-invalid @enderror">
                    ...
                    <option value="certificate" {{ old('type') == 'certificate' ? 'selected' : '' }}>Attestation</option>
            ...
            <div class="form-group">
                <label class="form-label"><i class="fas fa-heading"></i> Titre <span style="color:var(--primary);">*</span></label>
                <input type="text" name="title" class="form-input @error('title') is-invalid @enderror" required 
                       placeholder="Ex: Contrat de travail, Avenant..." value="{{ old('title') }}">
                @error('title') <span style="color:#EF4444; font-size:11px;">{{ $message }}</span> @enderror
            </div>
            <div class="form-group">
                <label class="form-label"><i class="fas fa-file"></i> Fichier <span style="color:var(--primary);">*</span></label>
                <input type="file" name="file" class="form-input @error('file') is-invalid @enderror" required 
                       accept=".pdf,.jpg,.jpeg,.png,.doc,.docx">
                <small style="color: var(--gray-600); font-size: 11px;">Formats acceptés : PDF, JPG, PNG, DOCX — max 5 Mo</small>
                @error('file') <span style="color:#EF4444; font-size:11px;">{{ $message }}</span> @enderror
            </div>
            <button type="submit" class="btn-primary">
                <i class="fas fa-cloud-upload-alt"></i> Téléverser le document
            </button>
        </form>
    </div>
    ...
    <div class="guide-card animate-in delay-2">
        ...
        <div class="guide-item">
            ...
            <div class="guide-text">
                <strong>Choisissez l'employé</strong>
                <p>Seuls les employés actifs de votre entreprise apparaissent.</p>
            </div>
        </div>
        <div class="guide-item">
            <div class="guide-icon"><i class="fas fa-tag"></i></div>
            <div class="guide-text">
                <strong>Définissez le type</strong>
                <p>Contrat, attestation ou autre document administratif.</p>
            </div>
        </div>
        <div class="guide-item">
            <div class="guide-icon"><i class="fas fa-heading"></i></div>
            <div class="guide-text">
                <strong>Donnez un titre</strong>
                <p>Un nom clair pour retrouver facilement le document.</p>
            </div>
        </div>
        <div class="guide-item">
            <div class="guide-icon"><i class="fas fa-file"></i></div>
            <div class="guide-text">
                <strong>Importez le fichier</strong>
                <p>Formats PDF, image ou Word acceptés (max 5 Mo).</p>
            </div>
        </div>
    </div>
</div>
