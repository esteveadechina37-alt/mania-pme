...
<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:28px;">
    <div>
        ...
        <p style="color:#6B6B6B; margin-top:6px;">Mettre à jour les informations de <strong>{{ $leaveType->name }}</strong></p>
    </div>
...
<form method="POST" action="{{ route('admin.leave-types.update', $leaveType) }}" style="background:#fff; border-radius:16px; padding:32px; box-shadow:0 2px 8px rgba(0,0,0,0.04); max-width:640px;">
    @csrf
    @method('PUT')

    <div style="margin-bottom:20px;">
        <label class="form-label" for="name">
            <i class="fas fa-tag" style="color:#FF6200; margin-right:6px;"></i> Nom *
        </label>
        <input type="text" name="name" id="name" class="form-input @error('name') is-invalid @enderror" value="{{ old('name', $leaveType->name) }}" required>
        @error('name')
    ...
    <div style="margin-bottom:20px;">
        ...
        <input type="number" name="days_allowed" id="days_allowed" class="form-input @error('days_allowed') is-invalid @enderror" value="{{ old('days_allowed', $leaveType->days_allowed) }}" required min="1">
        @error('days_allowed')
            <span class="error-text">{{ $message }}</span>
        @enderror
    </div>
