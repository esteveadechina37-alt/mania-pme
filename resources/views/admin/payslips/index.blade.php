...
<div class="page-header animate-in">
    <div>
        <h1 class="page-title"><i class="fas fa-file-invoice" style="color:var(--primary);"></i> <span>Bulletins de paie</span></h1>
    ...
    <div class="table-card animate-in delay-1">
        @if($payslips->count())
            <table class="premium-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="font-weight:600;">{{ $p->employee->user->name }}</td>
                        <td>{{ $p->month }} {{ $p->year }}</td>
                        <td style="font-weight:600;">{{ number_format($p->net_salary, 0, ',', ' ') }} FCFA</td>
                        <td style="text-align:right;">
                            <a href="{{ route('admin.payslips.download', $p) }}" class="action-btn" title="Télécharger">
                                <i class="fas fa-download"></i>
                            </a>
                            <form action="{{ route('admin.payslips.destroy', $p) }}" method="POST" style="display:inline;">
                                @csrf @method('DELETE')
                                <button type="button" onclick="openConfirmModal('{{ route('admin.payslips.destroy', $p) }}')" class="action-btn delete" title="Supprimer">
        ...
        @else
            <div style="text-align:center; padding:60px 20px; color:var(--gray-600);">
                ...
                <p style="font-size:16px; font-weight:500;">Aucun bulletin de paie pour le moment.</p>
            </div>
        @endif
    </div>
    ...
    <div class="guide-card animate-in delay-2" style="position: sticky; top: 100px;">
        ...
        <div class="guide-item">
            ...
            <div class="guide-text">
                <strong>Générer un bulletin</strong>
                <p>Cliquez sur "Nouveau bulletin", choisissez un employé et la période. Le salaire de base est automatique.</p>
            </div>
        </div>
        <div class="guide-item">
            ...
            <div class="guide-text">
                ...
                <p>Chaque bulletin peut être téléchargé en PDF pour archivage ou remise à l'employé.</p>
            </div>
        </div>
        <div class="guide-item">
            ...
            <div class="guide-text">
                ...
                <p>Les bulletins sont isolés par entreprise. Chaque employé ne voit que les siens.</p>
            </div>
        </div>
    </div>
</div>
